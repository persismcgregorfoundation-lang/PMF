...
const routes = ["home", "about", "projects", "reports", "donate", "gallery", "contact"] as const;
    ...
    <main className={`glass-app min-h-screen ${isDark ? "dark-theme" : "light-theme"}`}>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {route === "reports" && <ReportsPage isDark={isDark} />}
    ...
    <header className={`fixed left-0 top-0 z-[60] w-full px-3 transition-all duration-500 sm:px-6 ${isScrolled ? "py-2" : "py-4"}`}>
      <nav className={`glass-nav mx-auto flex max-w-7xl items-center justify-between rounded-full px-4 transition-all duration-500 sm:px-6 ${isScrolled ? "py-2 shadow-2xl" : "py-3"}`}>
        <a className="group flex items-center gap-3" href={routeHref("home")} aria-label="PERSIS McGREGOR FOUNDATION home">
          <span className="flex h-10 w-10 items-center justify-center rounded-full text-sm font-black text-white shadow-lg shadow-emerald-900/20 transition group-hover:scale-105" style={{ backgroundColor: primary }}>PMF</span>
        ...
        <div className="hidden items-center gap-7 lg:flex">
          ...
          <a className="rounded-full px-5 py-3 text-sm font-extrabold text-[#0f172a] shadow-lg shadow-amber-500/20 transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-amber-500/30" href={routeHref("donate")} style={{ backgroundColor: accent }}>Donate Now</a>
        </div>
      ...
      {isOpen && (
        <motion.div className="glass-menu mx-2 mt-3 rounded-3xl p-5 shadow-2xl lg:hidden" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="grid gap-2">
            ...
            <a className="rounded-2xl px-4 py-3 font-extrabold text-white" href={routeHref("donate")} style={{ backgroundColor: primary }}>Donate Now</a>
          </div>
        </motion.div>
      )}
    </header>
...
function ReportsPage({ isDark }: { isDark: boolean }) {
  ...
  const reports = [
    {
      ...
      description: "Detailed overview of meals provided, child-headed households assisted, and financial transparency.",
      file: "https://raw.githubusercontent.com/mozilla/pdf.js/ba2edeae/web/compressed.tracemonkey-pldi-09.pdf",
      date: "January 2026",
      category: "Impact & Finance"
    },
    {
      title: "Clean Water Initiative Strategy",
      description: "Technical review of community water well installations, maintenance schedules, and future expansions.",
      file: "https://s3-us-west-2.amazonaws.com/s.cdpn.io/149125/relativity.pdf",
      date: "November 2025",
      category: "Sustainability"
    },
    {
      title: "Primary Education Audit",
      description: "Evaluation of school readiness materials, uniform distributions, and academic retention metrics.",
      file: "https://raw.githubusercontent.com/mozilla/pdf.js/ba2edeae/web/compressed.tracemonkey-pldi-09.pdf",
      date: "August 2025",
      category: "Education"
    }
  ...
  const currentFile = customPdfUrl || reports[activeReportIndex].file;
  const currentTitle = customPdfUrl ? "Custom Loaded Document" : reports[activeReportIndex].title;

  function handleCustomLoad(e: React.FormEvent) {
    e.preventDefault();
    if (inputUrl.trim()) {
      setCustomPdfUrl(inputUrl.trim());
    }
  }

  return (
    <>
      <PageHero title="Official Reports" subtitle="Explore our latest impact assessments, financial audits, and strategic initiatives directly in our live viewer." />
      ...
      <section className="px-5 py-12 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mb-10 grid gap-4 md:grid-cols-3">
            {reports.map((report, idx) => (
              <button
                ...
                className={`flex flex-col text-left rounded-2xl border p-5 transition-all ${
                  !customPdfUrl && activeReportIndex === idx
                    ? 'border-[#0B6E4F] bg-[#0B6E4F]/5 shadow-[0_0_20px_rgba(11,110,79,0.15)] ring-1 ring-[#0B6E4F]/20'
                    : 'border-current/10 hover:border-current/30'
                }`}
              >
                <div className="flex items-center justify-between w-full">
                  <span className="text-[11px] font-extrabold uppercase tracking-wider text-[#0B6E4F]">
                    {report.category}
                  </span>
                  <span className="text-[11px] text-current/60 font-semibold">
                    {report.date}
                  </span>
                </div>
                ...
                <p className="mt-1 text-xs text-current/70 line-clamp-2">
                  {report.description}
                </p>
              </button>
            ))}
          ...
          {/* Optional URL Loader */}
          <div className="mb-8 flex flex-col sm:flex-row items-center justify-between gap-4 rounded-xl border border-current/10 bg-current/5 p-4">
            <div className="text-left w-full sm:w-auto">
              <span className="text-xs font-bold block">Test Your Own PDF</span>
              <span className="text-[11px] text-current/60 block">Enter a secure direct PDF link to view it instantly</span>
            </div>
            <form onSubmit={handleCustomLoad} className="flex w-full sm:w-auto gap-2">
              <input
                type="url"
                placeholder="https://example.com/file.pdf"
                value={inputUrl}
                onChange={e => setInputUrl(e.target.value)}
                className="w-full sm:w-64 rounded-lg border border-current/20 bg-white/50 px-3 py-1.5 text-xs outline-none focus:border-[#0B6E4F]"
              />
              <button
                type="submit"
                className="rounded-lg bg-[#0B6E4F] px-4 py-1.5 text-xs font-bold text-white hover:bg-[#07583f] whitespace-nowrap"
              >
                Load
              </button>
              {customPdfUrl && (
                <button
                  ...
                  className="rounded-lg border border-current/20 px-3 py-1.5 text-xs font-bold hover:bg-current/10"
                >
                  Clear
                </button>
              )}
